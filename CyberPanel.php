...
class Server_Manager_Openpanel extends Server_Manager
{
    ...
    public function init(): void
    {
        ...
        if (empty($this->_config['password'])) {
            throw new Server_Exception('The ":server_manager" server manager is not fully configured. Please configure the :missing', [':server_manager' => 'OpenPanel', ':missing' => 'password'], 2001);
        }
    }
    ...
    {
        throw new Server_Exception(':type: does not support :action:', [':type:' => 'OpenPanel', ':action:' => __trans('changing the account domain')]);
    }

    public function changeAccountIp(Server_Account $account, string $newIp): never
    {
        throw new Server_Exception(':type: does not support :action:', [':type:' => 'OpenPanel', ':action:' => __trans('changing the account IP')]);
    }
    ...
    {
        throw new Server_Exception(':type: does not support :action:', [':type:' => 'OpenPanel', ':action:' => __trans('username changes')]);
    }
    ...
    public function createAccount(Server_Account $account): bool
    {
        ...
        // If the `package` custom value is set, use that package from the OpenPanel server instead of implicitly creating a new one
        if (!empty($package->getCustomValue('package'))) {
            $this->getLog()->info('Using OpenPanel package name: ' . $package->getCustomValue('package') . ', ignoring package settings');
        ...
        try {
            ...
            if (strtolower($e->getMessage()) == strtolower(sprintf('Server Manager OpenPanel Error: "%s" ', 'That domain already exists'))) {
                return true;
            } else {
    ...
    {
        throw new Server_Exception(':type: does not support :action:', [':type:' => 'OpenPanel', ':action:' => __trans('synchronizing the account')]);
    }
    ...
    public function unsuspendAccount(Server_Account $account): bool
    {
        ...
        if ($info['suspended'] == 'no') {
            throw new Server_Exception('Server Manager OpenPanel Error: "Account is not suspended"');
        }
    ...
    private function request(string $command, array $fields = [], bool $post = true, string $asUser = ''): array
    {
        ...
        $url = $protocol . $host . ':' . $this->getPort() . '/api/' . $command . '?' . $field_string;
        ...
        if (strlen(strstr($data, '<!doctype html>')) > 0 || strlen(strstr($data, 'OpenPanel Login')) > 0) {
            throw new Server_Exception('Failed to connect to the :type: server. Please verify your credentials and configuration', [':type:' => 'OpenPanel']);
        }
        ...
        if (strlen(strstr($data, "The request you've made cannot be executed because it does not exist in your authority level")) > 0) {
            throw new Server_Exception('Server Manager OpenPanel Error: "The request you have made cannot be executed because it does not exist in your authority level"');
        }
        ...
        if (isset($response['error']) && $response['error'] == 1) {
            $placeholders = [':action:' => $command, ':type:' => 'OpenPanel'];
            $this->getLog()->err('Failed to ' . $command . ' on the OpenPanel server: ' . $response['text'] . ': ' . $response['details']);
